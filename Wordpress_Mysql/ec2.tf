resource "aws_instance" "ins1" {
    count = var.use_asg_spot == true ? 0 : var.ha_az
    ...
    subnet_id = var.ec2_public_subnet == true ? data.terraform_remote_state.vpc.outputs.public_subnets[count.index] : data.terraform_remote_state.vpc.outputs.private_subnets[count.index] 
    ...
    key_name = var.custom_instance_keypair_name != null ? var.custom_instance_keypair_name : aws_key_pair.generated_key_ssh.key_name
}
